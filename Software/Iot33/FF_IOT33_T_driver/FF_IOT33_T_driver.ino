...
void loop()
{
  ...
  */
    ...
    if ((WiFi.status()!= 3) && (millis() - mqtt_reconnect_time) > 20000){
      ...
      if (!Remote_Control){
        float manual_control_set[2];
        //manual_control_set = Manual_Control();
        Manual_Control(manual_control_set);
        MCP42050_Resistor_out(manual_control_set[0]);
        PWM_out(manual_control_set[1]);
